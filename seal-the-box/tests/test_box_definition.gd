...
func _test_classic_box() -> void:
	...
	assert(box.win_threshold == 13, "classic win_threshold should be 13, got %d" % box.win_threshold)
...
func _test_high_odds_box() -> void:
	...
	assert(box.win_threshold == 10, "high_odds win_threshold should be 10, got %d" % box.win_threshold)
...
func _test_custom_box() -> void:
	...
	assert(box.win_threshold == 4, "custom win_threshold should be 4 (floori(15*0.30)=floori(4.5)=4), got %d" % box.win_threshold)
