...
# Preload static registry classes so they compile in headless --script test mode.
...
# Track whether any tabs were sealed in the current round (for revenant_tabs no-seal hook).
var _sealed_this_round: bool = false
# Sum of tab values sealed this round. Used by forced_full_commit to compute leftover pips.
...
func start_match(box: BoxDefinition) -> void:
	...
	# Phantoms are appended after real tabs; reset_with_decoys uses a trailing-count approach.
	...
	# then append match_pool_delta so storm_box bonus dice are included.
