...
- **ENTRY-axis boxes (slice-boxes-5-entry-effects):** BoxEntryEffects static class (scripts/match/box_entry_effects.gd) — on_box_entry(box_id, gs) dispatcher + has_entry_effect(box_id) + get_description(box_id). 3 new boxes: storm_box (on entry adds fixed d2 + d10 to match_pool_delta; round_limit -= 1), cleanse_box (on entry refills all ability charges to max; round_limit -= 2), borrowed_time (on entry: hp -= 1; round_limit += 1; only spawnable when hp ≥ 3 — CaseManager.get_box_for_match() lazily replaces it with another medium box when hp < 3). RoundManager.start_match() fires entry effects after reset_match() (so delta starts empty) and after BoxWinConditions sets round_limit (so -1/-2/+1 layer on top). Pool = BoxDiceAccess result + match_pool_delta appended. [!] badge priority: ROLL → WIN → DICE → ENTRY.
- **BHV-axis boxes (slice-boxes-6-tab-behavior):** BoxTabBehavior static class (scripts/match/box_tab_behavior.gd) — on_round_start, on_round_end, on_round_end_no_seal, on_seal, has_behavior, get_description. 9 BHV boxes: regrowing (hard — lowest sealed tab returns each round start), shuffler (medium — all tabs randomised 1-9 each round start), moving_targets (hard — 7 tabs shift range R1:1-7/R2:2-8/R3:3-9/R4+:4-10), fading_decoys (medium — 3 phantom tabs hide until round 3), rising_tide (hard — all tabs +1 per round end, ceiling 13), growing_pillars (hard — tabs 2;3;2;3;2;3;2;3, +1 per round end), clock_tabs (hard — lowest tab −2 per round; hits 0 → 1 HP damage + remove), revenant_tabs (medium — if sealed nothing, lowest sealed tab returns), mitosis (boss — sealed tab ≥6 spawns floor(value/2) new tab). TabBoard overhauled: TabData inner class (value+is_decoy), tabs_changed signal, mutation API (add_tab, remove_tab, change_tab_value_at, replace_all_tabs, reveal_and_vanish_decoys), read API (get_real_remaining, get_tab_data, get_real_sum, has_decoys). RoundManager: tab_behavior_changed signal; BHV hooks wired; GameState.tabs synced BEFORE tab_behavior_changed emits. match.gd: [!] badge priority now ROLL→WIN→DICE→ENTRY→BHV; greyed sealed tabs use _sealed_button_indices in-place approach; _bhv_rebuilt_since_select flag for mitosis edge case; Continue button hidden on BHV mutation pushing sum above threshold. Pool now **44 boxes** (9 easy / 15 medium / 16 hard / 4 boss).
- Tests: test_run_manager.gd (46 tests) + test_power_effects.gd (30 tests) + test_ability_library.gd (22 abilities) + test_case_manager.gd (15 tests) + test_crossroads.gd (8 tests) + test_tab_board.gd + test_dice_pool.gd + test_box_definition.gd + test_box_definitions.gd (44-box pool validation) + test_box_roll_modifiers.gd + test_box_win_conditions.gd (18 tests) + test_dice_access.gd + test_entry_effects.gd (23 tests) + test_tab_behavior.gd pass headless.
