...
- Dev menu (T key or DEV button): scrollable panels; "Win Current Match" (threshold), "Shut the Box (Critical Win)", "Give Power →" submenu (all 11 powers), "Give Ability →" submenu (all 14 pool abilities, fills first empty slot or overwrites slot 3), "Switch Dice →" (mid-match die swap, no match transition), "Win Entire Series", "Restart Run", "+10 HP (Dev)" shortcuts for playtesting
...
- Source boxes in data/boxes.csv (tier=boss): source_devil ("the Pact", tabs 1–9, threshold 11), source_cosmic ("the Veil", tabs 2–10, threshold 11), source_ghost ("the Anchor", tabs 1;3;5;6;7;8;9;11;13, threshold 13). get_by_tier("boss") returns all three. Boss pool shuffled each reset_run() so match 9/21/27 each get a unique boss box.
- **Roll Modifier Registry** (`scripts/match/box_roll_modifiers.gd`): hardcoded Dict mapping box_id → Callable. Two modifier types: (1) *Mutation-type* — `apply_dice_mutation(box_id, hand)` fires once in `commit_roll`, mutates `die.value` in-place (heavy_dice, weak_dice, exploding_ones, pair_swallows). (2) *Total-override type* — `compute_total(box_id, hand)` is a pure function called every time the roll total is needed (halving_box, doubling_box, high_die_doubles). `is_total_override(box_id)` distinguishes them. Registry moves to CSV when it hits ~20 entries.
- **7 ROLL modifier boxes** in data/boxes.csv (15 total): heavy_dice (medium, +1 each die, tabs 2–9), weak_dice (hard, -1 each die floor 1, tabs 1–9), halving_box (hard, total÷2, tabs 1–6), doubling_box (medium, total×2, tabs 4;6;8;10;12;14;16), exploding_ones (medium, 1s reroll+add chains depth-10, tabs 1–9), pair_swallows (medium, equal-value pairs merge into sum, tabs 1–9), high_die_doubles (medium, highest die counts ×2, tabs 1–9).
- RoundManager: `_apply_box_roll_modifier(hand)` and `_compute_roll_total(hand)` helpers; `commit_roll`, `attempt_seal`, and `use_ability` all use `_compute_roll_total` so total-override boxes correctly affect sealing validation.
- **Win Condition Registry** (`scripts/match/box_win_conditions.gd`): hardcoded Dict mapping box_id → Callable. Callable returns `null` (use default), `true` (win), `false` (suppress threshold), or `int` (threshold override). `has_override(box_id)` and `evaluate(box_id, tab_board, round, threshold)` are the public API. Wired into `RoundManager._check_win()` and `start_round()`. Uses GDScript `is bool` / `is int` checks to avoid int/bool type comparison errors.
- **2 WIN boxes** in data/boxes.csv (17 total): crit_only (hard, threshold win disabled — must shut the box), escalating_threshold (hard, threshold drops each round: R1≤30, R2≤25, R3≤20, R4+≤15). `get_escalating_threshold(round)` is a static helper on BoxWinConditions.
- crit_only: Continue button never appears (threshold_reached never emits because the callable returns false). Threshold column in CSV (44) is ignored at runtime.
- escalating_threshold: `start_round()` calls `_apply_win_condition_threshold_update()` which updates `GameState.win_threshold` each round; UI threshold label reads this and updates automatically.
- Dev menu: "Force Round →" sub-overlay (Round 1–4 buttons) to set GameState.round and update win_threshold mid-match for testing escalating_threshold.
- Tests: test_run_manager.gd (46 tests) + test_power_effects.gd (30 tests) + test_ability_library.gd (22 abilities) + test_case_manager.gd (15 tests) + test_crossroads.gd (8 tests) + test_tab_board.gd + test_dice_pool.gd + test_box_definition.gd + test_box_definitions.gd + test_box_roll_modifiers.gd (31 tests) + test_box_win_conditions.gd (14 tests) pass headless. Note: test_run_manager.gd has a pre-existing flaky test (`_test_charges_decrement`) that occasionally fails when a die rolls its max face; this predates this slice.
