...
- BoxDefinition Resource + BoxLibrary autoload parse data/boxes.csv (columns: id, name, tabs, win_threshold, tier, source_for); BoxLibrary.get_by_tier(tier) returns all boxes of that tier; **22 boxes total** (pool: 5 easy / 5 medium / 9 hard / 3 boss); easy: classic, low_evens, lopsided_giant, easy_starter, crowded_low; medium: high_odds, stairs, cluster_of_fours, triple_triplets, mirror_ladder; hard: compressed, five_nines, high_wall, exact_evens, prime_pyramid, den_of_sevens, ten_pillars, the_long_count, avalanche; boss: source_devil/source_cosmic/source_ghost (the 3 themed Source boxes); get_random_source() returns a random boss-tier box. NOTE: the_long_count and avalanche were tagged "Boss" in the design spec but ship as tier="hard" to preserve the Source-as-act-finale identity for boss slots — change to "boss" in playtest tuning if pool dilution is desired.
