...
- BoxDefinition Resource + BoxLibrary autoload parse data/boxes.csv (columns: id, name, tabs, win_threshold, tier, source_for); BoxLibrary.get_by_tier(tier) returns all boxes of that tier; **22 boxes total** (pool: 5 easy / 4 medium / 10 hard / 3 boss); easy: classic, low_evens, stairs, easy_starter, crowded_low; medium: lopsided_giant, cluster_of_twos, triple_triplets, mirror_ladder; hard: compressed, five_nines, high_wall, exact_evens, prime_pyramid, den_of_sevens, ten_pillars, the_long_count, avalanche, high_odds; boss: source_devil/source_cosmic/source_ghost (the 3 themed Source boxes); get_random_source() returns a random boss-tier box. NOTE: the_long_count and avalanche were tagged "Boss" in the design spec but ship as tier="hard" to preserve the Source-as-act-finale identity for boss slots — change to "boss" in playtest tuning if pool dilution is desired. Playtest tuning (2026-05-08): stairs moved easy→easy (was medium), lopsided_giant easy→medium, high_odds medium→hard, cluster_of_fours renamed cluster_of_twos (10×2 tabs, threshold 6), mirror_ladder gained center 6 tab (now 11 tabs, threshold 10).
...
- Tests: test_run_manager.gd (46 tests) + test_power_effects.gd (30 tests) + test_ability_library.gd (22 abilities) + test_case_manager.gd (15 tests) + test_crossroads.gd (8 tests) + test_tab_board.gd + test_dice_pool.gd + test_box_definition.gd + test_box_definitions.gd (22-box pool validation) pass headless.
