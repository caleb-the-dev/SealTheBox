...
- BoxDefinition Resource + BoxLibrary autoload parse data/boxes.csv (columns: id, name, tabs, win_threshold, tier, source_for); BoxLibrary.get_by_tier(tier) returns all boxes of that tier; **28 boxes total** (pool: 9 easy / 7 medium / 9 hard / 3 boss); easy: classic, low_evens, stairs, easy_starter, crowded_low, compressed, the Veil (source_cosmic), heavy_dice, exploding_ones; medium: lopsided_giant, cluster_of_twos, triple_triplets, mirror_ladder, avalanche, doubling_box, high_die_doubles; hard: high_wall, exact_evens, prime_pyramid, the_long_count, high_odds, weak_dice, halving_box, **crit_only, escalating_threshold**; boss: the Pact (source_devil, threshold 7), the Anchor (source_ghost, threshold 10), Den of Sevens (den_of_sevens, 7×7 tabs, final boss). Boss pool shuffled so matches 9/21/27 each draw a unique boss. NOTE: the Pact has same tabs as Classic (1–9) — flagged for future tab redesign to feel distinct. NOTE: the_long_count has 18 tabs; tab buttons auto-scale to small size for display.
- **BoxRollModifiers** static class (scripts/match/box_roll_modifiers.gd): 6 ROLL box modifiers (heavy_dice +1/die, weak_dice −1/die, halving_box ÷2, doubling_box ×2, exploding_ones 1s chain-reroll, high_die_doubles highest die ×2). Wired into RoundManager.commit_roll(). Die.modifier_tag field added — shows bottom-left on die buttons after rolling (e.g. "1→7", "×2"). [!] badge in HUD top-left identifies ROLL **and WIN** boxes; badge hue cycles slowly (color animation via _process delta loop); hover shows floating tooltip with rule description.
...
- Dev menu (T key or DEV button): scrollable panels; "Win Current Match" (threshold), "Shut the Box (Critical Win)", "Give Power →" submenu (all 11 powers), "Give Ability →" submenu (all 14 pool abilities, fills first empty slot or overwrites slot 3), "Switch Dice →" (mid-match die swap, no match transition), "Win Entire Series", "Restart Run", "+10 HP (Dev)", "Force Bounty Box →" (win current + start bounty_box), "Reset Marquee Set" (clear marquee_seen so bounty_box fires its entry power again)
...
- **DICE-axis boxes (slice-boxes-4-dice-access):** 6 new boxes in boxes.csv. BoxDiceAccess static class (scripts/match/box_dice_access.gd) handles pool overrides (single_die, locked_d8, locked_d4) and round-end hooks (tax_per_roll: -1 HP per round after R1; forced_full_commit: damage = leftover rolled pips at round end). bounty_box: marquee match, grants phoenix_down on first entry per run (hardcoded in BOUNTY_BOX_POWER_ID; chosen for failsafe drama). Pool override is read-only — persistent pool unchanged after match. GameState.marquee_seen dictionary tracks which once-per-run boxes have fired. CaseManager.reset_run() deduplication pass ensures bounty_box appears at most once per run.
- Tests: test_run_manager.gd (46 tests) + test_power_effects.gd (30 tests) + test_ability_library.gd (22 abilities) + test_case_manager.gd (15 tests) + test_crossroads.gd (8 tests) + test_tab_board.gd + test_dice_pool.gd + test_box_definition.gd + test_box_definitions.gd (34-box pool validation) + test_box_roll_modifiers.gd + test_box_win_conditions.gd (18 tests) + test_dice_access.gd pass headless.
