...
- GameState: hp=6, MAX_HP=6, starting pool=1d4+4d6+2d8 (7 dice), ability_hand=[null, null, random_ability], owned_powers=[], power_counters={}, pending_threshold_bonus=0, match_pool_delta=[] (transient per-match die additions, cleared each match), marquee_seen={} (once-per-run entry-effect tracking, cleared at reset_run)
- Dev menu (T key or DEV button): scrollable panels; "Win Current Match" (threshold), "Shut the Box (Critical Win)", "Give Power →" submenu (all 11 powers), "Give Ability →" submenu (all 14 pool abilities, fills first empty slot or overwrites slot 3), "Switch Dice →" (mid-match die swap, no match transition), "Win Entire Series", "Restart Run", "+10 HP (Dev)", "Force Storm Box →", "Force Cleanse Box →", "Force Borrowed Time →" shortcuts for playtesting
...
- **ENTRY-axis boxes (slice-boxes-5-entry-effects, on feature branch):** BoxEntryEffects static class (scripts/match/box_entry_effects.gd) — on_box_entry(box_id, gs) dispatcher + has_entry_effect(box_id) predicate. 3 new boxes: storm_box (on entry adds one random d4/d6/d8 to match_pool_delta; die has storm_temp=true flag; pool = persistent + delta; delta cleared next match), cleanse_box (on entry refills all ability charges to max; harder tabs), borrowed_time (on entry: hp -=1; round_limit +=1; only spawnable when hp ≥ 3 — CaseManager.get_box_for_match() lazily replaces it with another medium box when hp < 3). RoundManager.start_match() fires entry effects before DicePool.setup() so delta is populated first.
- Tests: test_run_manager.gd (46 tests) + test_power_effects.gd (30 tests) + test_ability_library.gd (22 abilities) + test_case_manager.gd (15 tests) + test_crossroads.gd (8 tests) + test_box_definition.gd + test_dice_pool.gd + test_tab_board.gd + test_entry_effects.gd (21 tests) pass headless.
