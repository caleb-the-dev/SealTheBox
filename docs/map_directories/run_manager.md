...
func handle_rotation_pick(chosen: AbilityData) -> void
    ...
    # If completed == 9 or 21: emits show_crossroads(completed).
...
## Crossroads Timing
After completing match 9 (end of Act 1) or match 21 (end of Act 2), `handle_rotation_pick()` emits `show_crossroads(completed)`. The player must pick Rest or Whetstone before the next match starts.
...
All other matches: `handle_rotation_pick()` calls `_start_next_match()` directly — no texture beat, no intermediate screen.
...
Threshold win (critical=false):
  handle_match_won(false)
    ...
    → handle_rotation_pick(chosen)
         ...
         → else: _start_next_match() directly
...
- **Dev "Win Entire Series" loop order:** `dev_win_match()` → `dev_skip_rotation()` → `dev_skip_crossroads()`. No texture step — handle_rotation_pick() now calls _start_next_match() directly for non-crossroads matches.
...
| Date | Change |
|------|--------|
| 2026-05-08 | Playtest refactor: removed show_texture_beat signal, _do_texture_beat(), handle_texture_done(), dev_skip_texture(). handle_rotation_pick() now calls _start_next_match() directly for non-crossroads matches. TextureRoller, VignetteLibrary, EventLibrary dependencies removed. |
| 2026-05-07 | feature/within-act-texture: added texture beat system (removed 2026-05-08). |
